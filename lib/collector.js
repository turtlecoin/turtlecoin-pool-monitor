...
class Collector extends EventEmitter {
  constructor (opts) {
    ...
    this.updateTimer.on('tick', () => {
      ...
      this.database.cleanPollingHistory(cutoff)
...
function getPoolBlocks (daemon, pool) {
  ...
  if (pool.type === 'forknote') {
    url = util.format('%slive_stats?breaker=%s', pool.api, (new Date()).getTime())
  } else if (pool.type === 'node.js') {
    url = util.format('%spool/blocks?page=0&limit=30&breaker=%s', pool.api, (new Date()).getTime())
  ...
  return fetch(url, payload)
    .then(response => {
      ...
      if (!response || response.error || response.length === 0) return blocks

      if (pool.type === 'other' && response.Code === 1) {
        if (!Array.isArray(response.MinedBlocks)) return blocks

        response.MinedBlocks.forEach(item => blocks.push({ hash: false, height: item.Height }))
      } else if (pool.type === 'forknote') {
        if (!response || !response.pool || !response.pool.blocks) return blocks

        response = response.pool.blocks

        for (var i = 0; i < response.length; i++) {
          const hash = response[i].split(':').slice(0, 1)
          const height = response[++i]

          blocks.push({ hash, height })
        }
      } else if (pool.type === 'node.js') {
        if (!Array.isArray(response)) return blocks

        response.forEach(item => blocks.push({ hash: item.hash, height: item.height }))
      }

      return blocks.splice(0, 30)
    })
    .then(blocks => {
      if (pool.type !== 'other') return blocks

      return getBlocksInfoByHeight(daemon, blocks)
    })
