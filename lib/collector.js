        ...
        if (pool.type === 'forknote') {
          ...
          if (response.height) {
            ...
            if (response.pool_statistics.collective) {
              pool.hashrate = parseInt(response.pool_statistics.collective.hashrate || response.pool_statistics.collective.hashRate)
              pool.miners = parseInt(response.pool_statistics.collective.miners)
              pool.lastBlock = parseInt(response.pool_statistics.collective.lastFoundBlock.ts) / 1000
            } else if (response.pool_statistics.hashrate || response.pool_statistics.hashRate) {
              pool.hashrate = parseInt(response.pool_statistics.hashrate || response.pool_statistics.hashRate)
            ...
            pool.fee = response.config.pplns_fee
            pool.minPayout = response.config.min_wallet_payout
            pool.donation = 0
          } else if (response.block_template) {
            pool.height = response.block_template.height
            pool.status = 1
          }
        } else if (pool.type === 'other') {
